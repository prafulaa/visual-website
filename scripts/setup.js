const fs = require('fs');
const path = require('path');

// Create textures directory if it doesn't exist
const texturesDir = path.join(process.cwd(), 'public', 'textures');
if (!fs.existsSync(texturesDir)) {
  console.log('Creating textures directory...');
  fs.mkdirSync(texturesDir, { recursive: true });
}

// List of required texture files
const requiredTextures = [
  'earth.jpg',
  'sun.jpg',
  'mercury.jpg',
  'venus.jpg',
  'mars.jpg',
  'jupiter.jpg',
  'saturn.jpg'
];

// Create placeholder files if they don't exist
requiredTextures.forEach(texture => {
  const texturePath = path.join(texturesDir, texture);
  if (!fs.existsSync(texturePath)) {
    console.log(`Creating placeholder for ${texture}...`);
    
    // Create a 1x1 transparent pixel as placeholder
    // This is just a minimal valid JPEG file
    const placeholderJpeg = Buffer.from([
      
    ]);
    
    fs.writeFileSync(texturePath, placeholderJpeg);
  }
});

console.log('Setup completed successfully!');
console.log('Note: These are placeholder textures. For the best experience, replace them with actual planetary textures.');
console.log('You can find free planetary textures from NASA\'s image library or sites like Solar System Scope.'); 